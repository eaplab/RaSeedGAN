...
def generate_pipeline_testing(root_folder, us, channels, noise):
    """
...
def generate_scaling_data(root_folder, nx, ny, us, n_samples_train, channels, noise):
    ...
    if channels == 2:
        ...
        for idx in tqdm(range(n_samples_train)):
            ...
            Tpiv[idx, :, :] = np.array(piv['Tptv']).T
            Tptv[idx, :, :] = np.array(ptv['Tptv']).T
            Flag[idx, :, :] = np.array(ptv['Flagptv']).T
...
def generate_tfrecords_testing(root_folder, us, n_samples_train, n_samples_test, max_samples_per_tf, channels, noise):
    """
...
def tf_parser_testing(rec, root_folder, us, channels, noise):
    ...
    return piv, ptv, dns, cbc, flag, xlr, ylr, xhr, yhr
